...
exports.postBook = async (req, res) => {
    ...
    if (req.body.hour > 8 && req.body.ampm == 1)
        return res.send({ message: "We are available bewteen 8:00AM to 8:00PM. Please select other time!", red: 1 });
    if(req.body.date < new Date())
        return res.send({ message: "Please book a slot from today. Please select other time!", red: 1 });
    ...
    const { messageId } = await courier.send({
        eventId: "AJ8YGY9X11MF3XKSYB0GVJ35MFKN",
